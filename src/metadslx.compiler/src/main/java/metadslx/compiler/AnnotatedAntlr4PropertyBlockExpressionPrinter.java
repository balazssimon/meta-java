...
class AnnotatedAntlr4PropertyBlockExpressionPrinter extends AnnotatedAntlr4PropertiesParserBaseVisitor<Object> {
	...
	public Object visitPropertyAssignment(PropertyAssignmentContext ctx) {
		...
		if (selectorCount > 1) {
			output.getCompiler().getDiagnostics().addError("The property reference cannot have multiple indexers.",
					output.getCompiler().getFileName(), this.getAntlr4TextSpan(qprop), false);
			return null;
		}
		...
		if (qprop.scope != null) {
			...
			if ("global".equals(scopeName)) {
				...
				output.getCompiler().getDiagnostics().addError("Unknown scope.", output.getCompiler().getFileName(),
						this.getAntlr4TextSpan(propSels.get(0).name), false);
			}
			...
			if (elem != null) {
				if (selectorCount == 1) {
					if (propSels.get(0).selector != null) {
						...
						output.incIndent();
						output.writeIndent();
						output.write("this.setValue(ctx.{0}.get({2}), \"{1}\", Lazy.create(() -> ",
								elem.getAccessorName(), propName, selName);
						closeScopes = 2;
					} else if (propSels.get(1).selector != null) {
						output.getCompiler().getDiagnostics().addError("Invalid selector.",
								output.getCompiler().getFileName(), this.getAntlr4TextSpan(propSels.get(1).selector),
								true);
					} else {
						output.getCompiler().getDiagnostics().addError("Invalid selector.",
								output.getCompiler().getFileName(), this.getAntlr4TextSpan(qprop), true);
					}
				} else {
					output.writeIndent();
					output.write("this.setValue(ctx.{0}, \"{1}\", Lazy.create(() -> ", elem.getAccessorName(),
							propName);
				}
				started = true;
				closeFunction = true;
			} else if ("this".equals(elemName)) {
				if (propSels.get(1).selector != null) {
					output.getCompiler().getDiagnostics().addError("Invalid selector.",
							output.getCompiler().getFileName(), this.getAntlr4TextSpan(propSels.get(1).selector), true);
				} else {
					output.writeIndent();
					output.write("this.setValue(ctx, \"{0}\", Lazy.create(() -> ", propName);
				}
				started = true;
				closeFunction = true;
			} else {
				output.getCompiler().getDiagnostics().addError("Unknown property ctx.",
						output.getCompiler().getFileName(), this.getAntlr4TextSpan(propSels.get(0).name), false);
			}
		} else if (propSels.size() == 1) {
			String elemName = propSels.get(0).name.getText();
			ParserRuleElement elem = this.getElement(elemName);
			if (elem != null) {
				output.getCompiler().getDiagnostics().addError("Cannot assign a value to an element.",
						output.getCompiler().getFileName(), this.getAntlr4TextSpan(propSels.get(0).name), false);
			} else {
				...
				if (propSels.get(0).selector != null) {
					String selName = propSels.get(0).selector.getText();
					output.writeIndent();
					output.write("this.setValue(ctx, \"{0}\", {1}, Lazy.create(() -> ", propName, selName);
				} else {
					output.writeIndent();
					output.write("this.setValue(ctx, \"{0}\", Lazy.create(() -> ", propName);
				}
			...
			output.getCompiler().getDiagnostics().addError("Cannot assign a property indirectly.",
					output.getCompiler().getFileName(), this.getAntlr4TextSpan(qprop), false);
		}
	...
	public Object visitQualifiedProperty(QualifiedPropertyContext ctx) {
		...
		if (qprop.scope != null) {
			...
			output.getCompiler().getDiagnostics().addError("Unknown property context.",
					output.getCompiler().getFileName(), this.getAntlr4TextSpan(qprop), false);
		}
		return null;
	}
