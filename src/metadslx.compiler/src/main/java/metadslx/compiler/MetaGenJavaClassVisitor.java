...
class MetaGenJavaClassVisitor extends MetaGenVisitor {
	...
	public Object visitConfigDeclaration(ConfigDeclarationContext ctx) {
        ...
        for (ParseTree child: ctx.children)
        {
            ...
            if (child instanceof ConfigPropertyGroupDeclarationContext)
            {
                ...
                writeLine("this.{0} = new __{1}();", toCamelCase(prop.identifier().getText()), prop.identifier().getText());
            }
        }
        decIndent();
        writeLine("}");
    ...
    private boolean isTemplateOutputExpression(TemplateStatementStartEndContext statementStartEnd)
    {
        ...
        {
            ExpressionContext expression = statement.expressionStatement().expression();
            return !(expression instanceof LambdaExpressionContext || expression instanceof AssignmentExpressionContext);
        }
        return false;
    }
    ...
    public Object visitLoopStatementBegin(LoopStatementBeginContext ctx)
    {
        ...
        for (int i = 0; i < li.getItems().size(); i++)
        {
            ...
            {
                LoopChainTypeofExpressionContext expression = (LoopChainTypeofExpressionContext)lci.loopChainExpression();
                writeLine("__enumerate({1}).filter({0}_filter -> (Object){0}_filter instanceof {2}).map({0}_map -> ({2})(Object){0}_map).forEach({0} -> {3} {4}", lii.getName(), i > 0 ? ResolveIdentifier(li.getItems().get(i - 1).getName()) : "", expression.typeReference().getText(), "{", toComment(lci));
                incIndent();
            }
            else if (i == 0)
            {
            ...
            else
            {
            	...
            	write("__enumerate({1}.");
            	visit(lci);
            	write(").forEach({0} -> {2} {3}", lii.getName(), i > 0 ? ResolveIdentifier(li.getItems().get(i - 1).getName()) : "", "{", toComment(lci));
            	appendLine();
                incIndent();
            }
        }
        ...
        if (ctx.loopRunExpression() != null)
        {
            ...
            for (int i = lre.getChildCount() - 1; i >= 2; --i)
            {
                ...
                if (child instanceof LoopRunListContext)
                {
                    ...
                    if (i == lre.getChildCount() - 1) writeIndent();
    ...
    public Object visitHasLoopExpression(HasLoopExpressionContext ctx)
    {
        ...
        for (int i = 0; i < li.getItems().size(); ++i) {
            ...
            {
                LoopChainTypeofExpressionContext expression = (LoopChainTypeofExpressionContext)lci.loopChainExpression();
            	write("__enumerate({1}).filter({0}_filter -> (Object){0}_filter instanceof {2}).map({0}_map -> ({2})(Object){0}_map).anyMatch({0} -> {3}", lii.getName(), i > 0 ? ResolveIdentifier(li.getItems().get(i - 1).getName()) : "", expression.typeReference().getText(), toComment(lci));
            	appendLine();
                incIndent();
            }
            else if (i == 0)
            ...
            else
            {
            	write("__enumerate({1}.");
            	visit(lci);
            	write(").anyMatch({0} -> {2}", lii.getName(), i > 0 ? ResolveIdentifier(li.getItems().get(i - 1).getName()) : "", toComment(lci));
            	appendLine();
                incIndent();
            }
        }
