...
public class Antlr4AnnotationVisitor extends AnnotatedAntlr4ParserBaseVisitor<Object> {
    ...
    public Object visitLabeledAlt(LabeledAltContext ctx) {
        ...
        else
        {
            if (ctx.propertiesBlock() != null)
            {
                ...
                {
                    this.compiler.getDiagnostics().addError("There are multiple property blocks for the rule.", this.compiler.getFileName(), new Antlr4TextSpan(ctx.getParent()), false);
                }
    ...
    public Object visitAtom(AtomContext ctx) {
        ...
        if (ctx.terminal() != null)
        {
            ...
            {
                name = ctx.terminal().STRING_LITERAL().getText();
            }*/
        }
        if (ctx.ruleref() != null)
        {
        ...
        if (name != null)
        {
            ...
            if (ctx.getParent() instanceof AnnotatedAntlr4Parser.ElementContext)
            {
                ...
                this.currentElement.setIsArray(element.ebnfSuffix() != null && (element.ebnfSuffix().PLUS() != null || element.ebnfSuffix().STAR() != null));
            }
            else
            {
                if (ctx.getParent() instanceof AnnotatedAntlr4Parser.LabeledElementContext)
                {
                    AnnotatedAntlr4Parser.LabeledElementContext labeledElement = (AnnotatedAntlr4Parser.LabeledElementContext)ctx.getParent();
                    this.currentElement = new ParserRuleElement();
            ...
            if (this.currentElement != null)
            {
                ...
                {
                	Optional<ParserRuleElement> optOldElement = this.currentParserRuleAlt.getElements().stream().filter(e -> e.getName().equals(currentElement.getName())).findFirst();
                ...
                {
                	Optional<ParserRuleElement> optOldElement = this.currentParserRule.getElements().stream().filter(e -> e.getName().equals(currentElement.getName())).findFirst();
                    ParserRuleElement oldElement = null;
                    if (optOldElement.isPresent()) {
                    	oldElement = optOldElement.get();
                    }
    ...
    {
        if (rule == null) return;
        Optional<Annotation> autoSymbol = rule.getAnnotations().stream().filter(a -> "AutoSymbol".equals(a.getType().getName())).findFirst();
    ...
    {
        if (rule == null) return;
        Optional<Annotation> autoProperty = rule.getAnnotations().stream().filter(a -> "AutoProperty".equals(a.getType().getName())).findFirst();
    ...
    private void createPropertyAnnotations(ParserRule rule)
    {
        for (ParserRuleElement elem: rule.getElements())
        {
            ...
            {
                if (elem.getAnnotations().stream().filter(a -> "Property".equals(a.getType().getName())).findAny().isPresent()) continue;
    ...
    {
        Optional<AnnotationType> annotationTypeOpt = this.annotationTypes.stream().filter(at -> name.equals(at.getName())).findFirst();
    ...
    private void collectAnnotations(Iterable<AnnotatedAntlr4Parser.AnnotationContext> annotations)
    {
        for (AnnotatedAntlr4Parser.AnnotationContext annot: annotations)
        {
            ...
            {
                this.compiler.getDiagnostics().addWarning("No parent for annotation: " + annotation.getType().getName(), null, new Antlr4TextSpan(annot), false);
            }
            if (annot.annotationBody() != null)
            {
                if (annot.annotationBody().annotationAttributeList() != null)
                {
                    for (AnnotationAttributeContext attr: annot.annotationBody().annotationAttributeList().annotationAttribute())
                    {
    ...
    {
        if (ruleName == null || "".equals(ruleName)) return ruleName;
    ...
    protected String toValue(String value, boolean dynamic)
    {
        ...
        else
        {
            ...
            {
                isIdentifier = isIdentifier && (Character.isJavaIdentifierPart(value.charAt(i)) || value.charAt(i) == '.');
                if (value.charAt(i) == '.') ++dotCount;
                ++i;
            }
            if (isIdentifier)
            {
    ...
    private void generateAnnotatorVisitor()
    {
        ...
        for (ParserRule rule: currentGrammar.getParserRules())
        {
            for (Annotation annot: rule.getAnnotations())
            {
                ...
                {
                    writeLine("private {2} {0}_{1};", toCamelCase(rule.getName()), annot.getType().getName(), toAnnotationName(annot.getType().getName()));
                }
            }
            for (ParserRuleElement elem: rule.getElements())
            {
                for (Annotation annot: elem.getAnnotations())
                {
                    if (!annot.getType().isDynamic())
                    {
                        writeLine("private {3} {0}_{1}_{2};", toCamelCase(rule.getName()), toPascalCase(elem.getName()), annot.getType().getName(), toAnnotationName(annot.getType().getName()));
                    }
                }
            }
            for (ParserRule alt: rule.getAlternatives())
            {
                for (Annotation annot: alt.getAnnotations())
                {
                    if (!annot.getType().isDynamic())
                    {
                        writeLine("private {2} {0}_{1};", toCamelCase(alt.getName()), annot.getType().getName(), toAnnotationName(annot.getType().getName()));
                    }
                }
                for (ParserRuleElement elem: alt.getElements())
                {
                    for (Annotation annot: elem.getAnnotations())
                    {
                        if (!annot.getType().isDynamic())
                        {
                            writeLine("private {3} {0}_{1}_{2};", toCamelCase(alt.getName()), toPascalCase(elem.getName()), annot.getType().getName(), toAnnotationName(annot.getType().getName()));
                        }
                    }
                }
            }
        ...
        for (ParserRule rule: currentGrammar.getParserRules())
        {
            if (rule.getAnnotations().stream().anyMatch(a -> !a.getType().isDynamic()) || rule.getElements().stream().anyMatch(e -> e.getAnnotations().stream().anyMatch(a -> !a.getType().isDynamic())))
            {
                writeLine();
            }
            ...
            for (ParserRuleElement elem: rule.getElements())
            {
                for (Annotation annot: elem.getAnnotations())
                {
                    if (!annot.getType().isDynamic())
                    {
                        String tmp = MessageFormat.format("this.{0}_{1}_{2}", toCamelCase(rule.getName()), toPascalCase(elem.getName()), annot.getType().getName());
                        this.generateAnnotationCreation(annot, tmp, false);
                    }
                }
            }
            for (ParserRule alt: rule.getAlternatives())
            {
                if (rule.getAnnotations().stream().anyMatch(a -> !a.getType().isDynamic()) || alt.getAnnotations().stream().anyMatch(a -> !a.getType().isDynamic()) || alt.getElements().stream().anyMatch(e -> e.getAnnotations().stream().anyMatch(a -> !a.getType().isDynamic())))
                {
                    writeLine();
                }
                ...
                for (ParserRuleElement elem: alt.getElements())
                {
                    for (Annotation annot: elem.getAnnotations())
                    {
                        if (!annot.getType().isDynamic())
                        {
                            String tmp = MessageFormat.format("this.{0}_{1}_{2}", toCamelCase(alt.getName()), toPascalCase(elem.getName()), annot.getType().getName());
                            this.generateAnnotationCreation(annot, tmp, false);
                        }
                    }
                }
    ...
    public static class ParserRule {
		...
		public boolean HasElementAnnotations()
        {
            ...
            {
                if (elem.getAnnotations().size() > 0) return true;
            }
            return false;
        }
    }
    public static class ParserRuleElement {
        ...
        {
            String prefix = "";
            if (reservedNames.contains(this.name)) prefix = "_";
    ...
    private static final String[] reservedNamesArray =
    {
    	"abstract",
    	"continue",
    	"for",
