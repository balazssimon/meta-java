...
public class Antlr4AnnotationRemover extends AnnotatedAntlr4ParserBaseVisitor<Object> {
    ...
    public Antlr4TextSpan getTokenSpanAt(int line, int column)
    {
        ...
        {
            if (token.getLine() == line && token.getCharPositionInLine() == column) return new Antlr4TextSpan(token);
        }
    ...
    public Object visitOption(OptionContext ctx) {
        if (ctx.id().getText() == "generateCompiler")
        {
            this.removeText(ctx);
        }
        else if (ctx.id().getText() == "generateCompilerBase")
        {
            this.removeText(ctx);
        }
        return null;
